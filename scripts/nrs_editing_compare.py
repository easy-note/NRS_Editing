...
def extract_video_marking_clip(video_path, results_dir, mark_start_time, mark_end_time, padding_sec, mark):
    from core.utils.ffmpegHelper import ffmpegHelper
    from core.utils.misc import idx_to_time, time_to_sec, sec_to_time

    # calc paddingsec
    start_sec, end_sec = time_to_sec(mark_start_time) - padding_sec , time_to_sec(mark_end_time) + padding_sec
    start_time, end_time = sec_to_time(start_sec), sec_to_time(end_sec)

    # formating ms
    hms, ms = start_time.split('.')
    ms = ms[:2]
    start_time = '.'.join([hms, ms])

    hms, ms = end_time.split('.')
    ms = ms[:2]
    end_time = '.'.join([hms, ms])

    hms, ms = mark_start_time.split('.')
    ms = ms[:2]
    mark_start_time = '.'.join([hms, ms])

    hms, ms = mark_end_time.split('.')
    ms = ms[:2]
    mark_end_time = '.'.join([hms, ms])

    # set save file name
    save_name = '{}-{}[{}-{}]'.format(start_time, end_time, mark_start_time, mark_end_time)

    ffmpeg_helper = ffmpegHelper(video_path, results_dir)
    ffmpeg_helper.extract_video_marking_clip(start_time, end_time, time_to_sec(mark_start_time), time_to_sec(mark_end_time), save_name, mark)
...
## 5. 22.03.15 hg, extract from json
def extract_video_from_json(video_path, json_path, editted_video_dir, video_fps, padding_sec=3):
    import os
    from core.utils.misc import idx_to_time
    ...
    # temp process path
    padding_frame = int(padding_sec * video_fps)

    print('\n\n \t\t <<<<< EXTRACTING CLIPS >>>>> \t\t \n\n')
    for i, (start_idx, end_idx, code) in enumerate(target_clipping_time, 1):

        process_dir = os.path.join(editted_video_dir, '{}'.format(code), '{}-{}'.format(start_idx, end_idx))
        
        os.makedirs(process_dir, exist_ok=True)

        mark_start_time, mark_end_time = idx_to_time(start_idx, video_fps), idx_to_time(end_idx, video_fps) # conver to time

        print('\n\n[{}] \t {} - {} - {}'.format(i, mark_start_time, mark_end_time, code))
        
        # extract_video_marking_clip(video_path, temp_clip_dir, start_time, end_time, pre_start_time, post_end_time, mark=code)
...
def return_gt(video_name):
    import os
    import glob

    gt_paths = glob.glob(os.path.join('/nas1/OOB_Project/annotation/Gastrectomy/Lapa/v3', '*', '*', '*.json'))

    for gt_path in gt_paths:
        if gt_path.find(video_name) != -1:
            return gt_path
...
def main():
    ...
    input_path = '/nas1/ViHUB-pro/input_video/ViHUB_0315'
    ...
    for (root, dirs, files) in os.walk(input_path):
        ...
        for file in files:
            ...
            report_annotation_save_path = os.path.join(inference_json_output_path, '{}-annotation_by_inference.json'.format(video_name))
            report_annotation(frameRate, totalFrame, width, height, video_name, event_sequence, inference_interval, result_save_path=report_annotation_save_path)
            ...
            #### compare gt ####
            gt_path = return_gt(video_name)
            print('>>>>>>> COMPARE GT <<<<<<< \t video_name: {} | gt_path : {}'.format(video_name, gt_path))

            compare_save_path = os.path.join(inference_json_output_path, '{}-compare.json'.format(video_name))
            qa_helper = QAHelper(target_video, gt_path, report_annotation_save_path, inference_interval, compare_save_path)

            qa_helper.compare_to_json()

            #### extract marking clips ####
            extract_video_from_json(video_path=target_video, json_path=compare_save_path, editted_video_dir=os.path.join(inference_json_output_path, 'marking'), video_fps=frameRate, padding_sec=3)
...
if __name__ == '__main__':
    if __package__ is None:
        ...
        from core.utils.QAHelper import QAHelper
